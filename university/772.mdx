...
* Pass in any options and configuration.
* Create a game ID.
* Store this in DynamoDB and hold game loop data in ElasticCache (redis).
* Once they have created a game, they cannot edit the game options after that.

#### Fetch Game

* Call via game ID.
* Return all data related to a game.
* Building export data on client.
* Send all data as JSON, client will format it over to CSV.
...
```json
{
    ...
    data: {
        ...
        round: number
        data: {
            // Player Interface with updated data
        }
    }
}
```
Store `request.data` in Elastic Cache, just writing to it. Once we move to the next round, we will pull everything from the previous round from the ElasticCache, as an array, process it and do one big update to DynamoDB of all the updated objects.