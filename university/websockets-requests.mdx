...
`${planetID}-${gameID}`: {
    planetID: "planet1",
    planet_name: `${AI generated}`,
    taxRate: data.defaultRate,
    players: [{
...
```javascript
    statusCode: 200,
    body: `{"type":"createGame","message":["OK","OK","OK"]}`
```
...
```javascript
    statusCode: 200,
    body: `{"type":"joinGame","message":{"options":{"stageTime":20,"auditChance":0.25,"finePercent":0.15,"planetCollectionMultiplier":1.7,"taxRate":0.3,"declaredIncomePercent":0.4,"incomeMax":1200,"incomeMin":600,"bankruptcyThreshold":0,"generateNewIncomeEachRound":false}}}`
```

<Note>Each time a player sends a joinGame request, the host will receive the following string: `{"playerID":"001a","playerName":"Emma", "type":"[new|rejoin]"}`</Note>
...
`{"type":"startGame","message":{"planetID":"planet1","planet_name":"Mystoria","playerRole":"A","planet_members":["Emma","Jules","Stevie","Jules","Thomas"]}}`

### Host receives:
`{"type":"startGame","message":"OK"}`
...
```javascript
    statusCode: 200,
    body: `{"type":"startRound","message":{"round":1,"stage":1,"stageStartTime":"19/06/2024, 19:07:57","stageDuration":20}}`
```
...
* `handleStage` should be sent by the **Host** when the stage is up (stageDuration has elapsed)
    - This way, only one system is managing the time, and the WebSockets aren't using up a massive amount of processing power while each connection waits for stage duration to elapse before sending a response.
...
planetID: string
```
**Response:**  `{"type":"stageChanges","message":["OK","OK"]}`
...
newWealth: number,
audit: number, //ONLY FOR B AND C PLAYERS
redistributed: number, //ONLY FOR A PLAYERS. FOR THE RECORDS
```
...
```javascript
    statusCode: 200,
    body: `{"type":"handleStage","message":{"round":1,"stage":2,"stageStartTime":"19/06/2024, 19:10:21","stageDuration":20}}`
```

#### To Players - after stage 1
```javascript
    statusCode: 200,
    body: `{"type":"handleStage","message":{"round":1,"stage":2,"stageStartTime":"19/06/2024, 19:10:21","stageDuration":20,"taxRate":0.7}}`
```

#### To Players - after stage 2
**A Players | Not Bankrupt**
```javascript
    statusCode: 200,
    body: `{"type":"handleStage","message":{"round":1,"stage":3,"stageStartTime":"19/06/2024, 19:12:23","stageDuration":20,"bankrupt":false,"declared":[0,600],"redistributable":306}}`
```

**A Players | Bankrupt**
```javascript
    statusCode: 200,
    body: `{"type":"handleStage","message":{"round":1,"stage":3,"stageStartTime":"19/06/2024, 19:12:23","stageDuration":20,"bankrupt":true}}`
```

**Other Players**
```javascript
    statusCode: 200,
    body: `{"type":"handleStage","message":{"round":1,"stage":3,"stageStartTime":"19/06/2024, 16:21:12","stageDuration":20}}`
```
